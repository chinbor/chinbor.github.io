...
* 安装字体 `Hasklug Nerd Font Mono` 并设置为vscode的editor默认字体（这一步其实是非必须的，取决于你是否想要自定义字体）
...
首先你需要取消 vim 中的 `ctrl+key` 的快捷键映射关系，当然你不嫌麻烦可以将其所有的 `ctrl+key` 修改为自定义快捷键, 当然了由于vim走的粘贴复制并不是系统的剪贴板，所以你还需要允许其共享系统剪贴板，`CTRL + ,` 然后输入 `vim.use` 再进行如下勾选

![共存操作](./2022-11-18-11-31-54.png)
